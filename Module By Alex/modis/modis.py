...
def ymd_open_orbit_tracks(year, month, day):
	"根据年月日获取指定日期的轨道图"
	# 可以用来打开指定年月日的轨道图像
...
def monitoring_terra_data():
	"监测 Terra 数据，存在的话自动下载"
	# Open terra data download sites based on the current day.
	from PIL import ImageGrab
	from PIL import Image, ImageFilter
	import pynput, time, os, psutil, pytesseract, datetime, webbrowser, xml.dom.minidom, modis
	...
	for i in range(0, 100):
		print("Downloading " + "MOD021KM" + site_end)
		#openFirefox()
		...
		if(img):
			...
			if(flag_cha):
				...
				for MOD in MODS:
					...
					print("Downloading " + MOD + site_end)
					time.sleep(10)
					mouse.click(Button.left)
					modis.py33.closePage()
				print("----- Downloading -----")
...
def monitoring_aqua_data():
	...
	flag = 0
	for i in range(0, 100):
		print("Downloading " + "MYD021KM" + site_end)
		#openFirefox()
		...
		if(img):
			...
			if(text.find("保") > -1 or text.find("存") > -1):
				...
				for MYD in MYDS:
					...
					print("Downloading " + MYD + site_end)
					time.sleep(10)
					mouse.click(Button.left)
					modis.py33.closePage()
				print("----- Downloading -----")
...
def a():
	"简化操作函数"
	# 根据时间和相应参数智能调用函数
	import datetime, xml.dom.minidom, time, modis
	...
	if (today - date_date).days >= 1:
		...
		modis.py33.add_time(terra_input, aqua_input)
		# 执行监测Terra、Aqua监测函数
		if today.hour <= 12:
...
def ymd_browser_terra(year, month, day, str_time):
	"根据年月日通过浏览器下载指定日期的 Terra 数据"
	# Open terra data download sites based on the current day.
